...
from tinygrad.nn.state import gguf_load, torch_load, load_state_dict, get_state_dict, safe_load, safe_save
...
class GPT2:
    ...
    @staticmethod
    def load_local(model_size="gpt2", checkpoint_path=None):
        tokenizer = tiktoken.get_encoding("gpt2")
        model = Transformer(
            n_layers=12,
            n_heads=12,
            dim=768,
            norm_eps=1e-5,
            vocab_size=50257,  # Use padded_vocab_size
            max_seq_len=1024,
        )

        if checkpoint_path:
            print(f"loading checkpoint from {checkpoint_path}")
            weights = safe_load(checkpoint_path)
            load_state_dict(model, weights)
        ...
        return GPT2(model, tokenizer)  # We forgot this return statement!
...
if __name__ == "__main__":
    ...
    # Load our finetuned model by default
    if not args.use_pretrained and os.path.exists("checkpoints/gpt2_final.safetensors"):
        gpt2 = GPT2.load_local(args.model_size, "checkpoints/gpt2_final.safetensors")
        print("Using finetuned Shakespeare model")
    else:
        gpt2 = (
        ...
        if not args.use_pretrained:
            print("Finetuned model not found, using pretrained weights")
